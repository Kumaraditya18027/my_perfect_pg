...
const RoomDetailsForm = () => {
  ...
  const validateForm = () => {
    ...
    rooms.forEach((room, index) => {
      ...
      if (!room.rates.monthly)
        newErrors[`monthlyRate${index}`] = "Monthly rate is required";
    });
    if (services.fooding && !services.foodingType)
      newErrors.foodingType = "Fooding type is required";
    setErrors(newErrors);
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      router.push({
        pathname: "/pgowner/addOwnerDetails",
      ...
      <div className="max-w-3xl mx-auto">
        <form onSubmit={handleSubmit} className="space-y-6">
          {rooms.map((room, index) => (
            <div
              key={index}
              className="bg-white/80 backdrop-blur-sm shadow-lg rounded-lg p-6 relative"
            >
              {index > 0 && (
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-4">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleRoomChange(index, "features.ac", e.target.checked)
                      }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleRoomChange(index, "rates.monthly", e.target.value)
                    }
                    className="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <label className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) =>
                    handleServicesChange("fooding", e.target.checked)
                  }
                  className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
                />
              ...
              {services.fooding && (
                <select
                  ...
                  onChange={(e) =>
                    handleServicesChange("foodingType", e.target.value)
                  }
                  className="w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
                >
              ...
              {["ac", "cctv", "wifi", "laundry", "parking", "security"].map(
                (service) => (
