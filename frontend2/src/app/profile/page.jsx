...
export default function ProfilePage() {
  ...
  const [user, setUser] = useState({
    ...
    bio: "Looking for the perfect PG in Mumbai",
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {Object.entries(user.preferences).map(([key, value]) => (
              <div key={key} className={`${COLOR_SCHEME.primary.light} rounded-lg p-4`}>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="text-lg font-semibold">{user.currentPG.name}</h4>
                  <p className="text-gray-500 mt-1">{user.currentPG.address}</p>
                </div>
                <div className={`px-3 py-1 rounded-full ${COLOR_SCHEME.primary.light} ${COLOR_SCHEME.primary.text}`}>
                  {user.currentPG.rent}
                </div>
              </div>
              ...
              <div className="mt-4 flex items-center space-x-2">
                ...
                <span className="text-gray-500">• Joined {user.currentPG.joinedDate}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-500 text-sm mt-1">{bookmark.location}</p>
                    <div className={`mt-2 ${COLOR_SCHEME.primary.text} font-medium`}>
                      {bookmark.price}
                    </div>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm mx-4">
            ...
            <p className="text-gray-600">Are you sure you want to remove this PG from your bookmarks?</p>
            <div className="flex justify-end space-x-3 mt-6">
              <button
